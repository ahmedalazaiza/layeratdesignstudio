...
export function ProductDetailPage({
  ...
  const isInWishlist =
    ...
    (authUser?.wishlist?.includes(productId) ?? false);

  // Sync SEO & Structured Data Schema
  useEffect(() => {
    if (!product) return;
    const meta = generateSEOMetadata({ page: "product", productId }, product);
  ...
  const subcategory = category?.subcategories?.find(
    (s) => (s.id || (s as any)._id) === product.subcategoryId
  );

  const allImages = Array.from(
    new Set([product.thumbnail, ...(product.galleryImages || [])].filter(Boolean) as string[])
  );

  const galleryImages: string[] = allImages.length > 0 ? allImages : [product.thumbnail || ""];
